...
#define PARALLEL_KEY_INV_ROWS			UINT64CONST(0xA000000000000007)
...
_bt_begin_parallel(BTBuildState *buildstate, bool isconcurrent, int request)
{
	...
	/*
	 * Estimate space for WalUsage, BufferUsage and inv_rows -- PARALLEL_KEY_WAL_USAGE,
...
_bt_end_parallel(BTLeader *btleader)
{
	...
	for (i = 0; i < btleader->pcxt->nworkers_launched; i++)
		InstrAccumParallelQuery(&btleader->bufferusage[i], &btleader->walusage[i], &btleader->inv_rows[i]);
