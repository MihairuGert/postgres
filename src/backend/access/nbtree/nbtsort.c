...
#define PARALLEL_KEY_INV_ROWS_USAGE		UINT64CONST(0xA000000000000007)
...
_bt_begin_parallel(BTBuildState *buildstate, bool isconcurrent, int request)
{
	...
	btleader->invrowsusage = invrowsusage;
...
_bt_end_parallel(BTLeader *btleader)
{
	...
	for (i = 0; i < btleader->pcxt->nworkers_launched; i++)
		InstrAccumParallelQuery(&btleader->bufferusage[i], &btleader->walusage[i], &btleader->invrowsusage[i]);
