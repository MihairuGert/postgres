...
#define PARALLEL_VACUUM_KEY_INV_ROWS		6
					 ...
					 int elevel, BufferAccessStrategy bstrategy)
{
	...
	/*
	 * Estimate space for BufferUsage,WalUsage and invisible rows --
	 * PARALLEL_VACUUM_KEY_BUFFER_USAGE, PARALLEL_VACUUM_KEY_WAL_USAGE
	 * PARALLEL_VACUUM_KEY_INV_ROWS.
	 *
	...
	/*
	 * Allocate space for each worker's BufferUsage, WalUsage and invisible rows count; 
	...
	inv_rows = shm_toc_allocate(pcxt->toc,
								 mul_size(sizeof(uint64), pcxt->nworkers));
	shm_toc_insert(pcxt->toc, PARALLEL_VACUUM_KEY_INV_ROWS, inv_rows);
	pvs->inv_rows = inv_rows;
									...
									bool vacuum)
{
	...
	if (nworkers > 0)
	{
		...
		for (int i = 0; i < pvs->pcxt->nworkers_launched; i++)
			InstrAccumParallelQuery(&pvs->buffer_usage[i], &pvs->wal_usage[i], &pvs->inv_rows[i]);
	}
...
parallel_vacuum_main(dsm_segment *seg, shm_toc *toc)
{
	...
	inv_rows = shm_toc_lookup(toc, PARALLEL_VACUUM_KEY_INV_ROWS, false);
