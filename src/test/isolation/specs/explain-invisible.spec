...
session "explainer"  
step "s2_begin" { BEGIN; }
step "s2_explain" { EXPLAIN (ANALYZE, INV_ROWS, COSTS OFF, TIMING OFF, SUMMARY OFF, BUFFERS OFF) SELECT * FROM test_invisible_table_seq_scan; }
...
step "s2_explain_bitmap_and_index" { EXPLAIN (ANALYZE, INV_ROWS, COSTS OFF, TIMING OFF, SUMMARY OFF, BUFFERS OFF) 
                                     SELECT * FROM test_invisible_rows_bitmap_heap_and_index_scan 
                                     WHERE id >= 1 AND id <= 6000; }
step "s2_explain_index_only_covering" { EXPLAIN (ANALYZE, INV_ROWS, COSTS OFF, TIMING OFF, SUMMARY OFF, BUFFERS OFF)
                                        SELECT id, data FROM test_invisible_rows_bitmap_heap_and_index_scan 
                                        WHERE id >= 1 AND id <= 6000; }
step "s2_explain_parallel_scan" { 
    SET max_parallel_workers_per_gather = 4;
    EXPLAIN (ANALYZE, INV_ROWS, COSTS OFF, TIMING OFF, SUMMARY OFF, BUFFERS OFF) SELECT COUNT(*) FROM p1 join p2 on p1.c1 = p2.c1; 
    }
